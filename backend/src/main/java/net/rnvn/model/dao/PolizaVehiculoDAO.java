...
public class PolizaVehiculoDAO extends PolizasDAO {
    ...
    public PolizaVehiculo getPolizaVehiculo(int idPoliza) {
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                PolizaVehiculo.PlazoPago plazoPago = PolizaVehiculo.PlazoPago.fromString(rs.getString("plazo_pago"));
    ...
    public boolean actualizarPolizaVehiculo(PolizaVehiculo polizaVehiculo) {
        ...
        try (Connection conn = getConnection();
            ...
            stmt.clearParameters();
            stmt.setString(1, polizaVehiculo.getIdCliente());
            stmt.setString(2, polizaVehiculo.getPlacaVehiculo());
            stmt.setInt(3, polizaVehiculo.getIdCategoriaVehiculo());
            stmt.setBigDecimal(4, polizaVehiculo.getValorAsegurado());
            stmt.setDate(5, java.sql.Date.valueOf(polizaVehiculo.getFechaInicio().toLocalDate()));
            stmt.setString(6, polizaVehiculo.getPlazoPago().getCode());
            stmt.setInt(7, polizaVehiculo.getIdPoliza());
            result = stmt.executeUpdate() > 0;
        } catch (Exception e) {
            String className = this.getClass().getName();
    ...
    private static final String SQL_ADD_POLIZA_VEHICULO = QueryGen.genInsertInto(
            TABLE_NAME,
            new String[] {
                    "id_cliente",
                    "id_poliza",
                    ...
                    "valor_asegurado",
                    "fecha_inicio", "plazo_pago" });
    private static final String SQL_GET_POLIZA_VEHICULO = QueryGen.genSelectString(
            TABLE_NAME,
            new String[] {
                    "id_cliente",
                    "placa_vehiculo",
    ...
    private static final String SQL_UPDATE_POLIZA_VEHICULO = QueryGen.genUpdateString(
            TABLE_NAME,
            new String[] {
                    "id_cliente",
                    "placa_vehiculo",
                    ...
                    "plazo_pago" },
            new String[] { "id_poliza = ?" });
    private static final String SQL_DELETE_POLIZA_VEHICULO = QueryGen.genDeleteString(
            TABLE_NAME,
            new String[] { "id_poliza = ?" });
}
